...
- The solution is an ensemble of 5 transformer models, each trained on 5 folds: 2x `deberta-large`, 2x `deberta-v3-large` and 1x `longformer-large`.
...
- In order to use deberta v2 or v3, you need to patch transformers library to create a new fast tokenizer using data and instructions from this [kaggle kernel](https://www.kaggle.com/nbroad/deberta-v2-3-fast-tokenizer).
...
To train different models on different folds (0...4) make changes inside the train.sh file. The two version of deberta models were trained using `dropout=0.1` and `dropout=0.15` and `max_len=1024`, while for longformer-large model `dropout=0.1` and `max_len=1536` parameters were used. 
